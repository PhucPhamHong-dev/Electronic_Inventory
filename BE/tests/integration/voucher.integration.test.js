...
describeIntegration("VoucherService integration", () => {
    ...
    (0, vitest_1.beforeAll)(async () => {
        ...
        const serviceModule = await Promise.resolve().then(() => __importStar(require("../../src/services/VoucherService")));
    ...
    (0, vitest_1.it)("allows only one concurrent sales request for same stock", async () => {
        ...
        (0, vitest_1.expect)(failureCount).toBe(1);
        const reloaded = await prisma.product.findUniqueOrThrow({ where: { id: product.id } });
        (0, vitest_1.expect)(Number(reloaded.stockQuantity)).toBe(2);
    });
    (0, vitest_1.it)("reverses booked voucher impacts before applying updated values", async () => {
        const service = new VoucherServiceClass(prisma);
        const partner = await prisma.partner.create({
        ...
        await service.updateVoucher(created.voucherId, {
            partnerId: partner.id,
        ...
        (0, vitest_1.expect)(Number(updatedProduct.stockQuantity)).toBe(17);
        (0, vitest_1.expect)(Number(updatedPartner.currentDebt)).toBe(45);
        (0, vitest_1.expect)(updatedVoucher.isEdited).toBe(true);
    });
    (0, vitest_1.it)("rolls back entire transaction if one voucher item fails stock check", async () => {
        const service = new VoucherServiceClass(prisma);
        const partner = await prisma.partner.create({
        ...
        await (0, vitest_1.expect)(service.createSalesVoucher({
            partnerId: partner.id,
            items: [
                { productId: p1.id, quantity: 2, unitPrice: 5 },
                { productId: p2.id, quantity: 5, unitPrice: 5 }
            ]
        }, serviceContext)).rejects.toBeTruthy();
        ...
        (0, vitest_1.expect)(Number(p1After.stockQuantity)).toBe(5);
        (0, vitest_1.expect)(Number(p2After.stockQuantity)).toBe(1);
    });
    (0, vitest_1.it)("writes trigger-based audit log with old_value and new_value on voucher update", async () => {
        const service = new VoucherServiceClass(prisma);
        const partner = await prisma.partner.create({
        ...
        (0, vitest_1.expect)(auditRows.length).toBeGreaterThan(0);
        (0, vitest_1.expect)(auditRows.some((row) => row.oldValue !== null && row.newValue !== null)).toBe(true);
    });
});
